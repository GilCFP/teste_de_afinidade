...
def tester(arquivos, nomes): #efetua o teste de afinidade para n cursos
    ...
    for i in range(0, len(order)):
        questions[curso[order[i]][0].split("/")[1].replace(".txt","")].append(curso[order[i]][results[order[i]]
              [index[order[i]]]])
        index[order[i]] += 1
    return questions
...
def main():
    ...
    questions = tester(len(sys.argv), files)
    names = ['nothing'] * (len(sys.argv) - 1)
    names[0] = sys.argv[1]
    for i in range(2,len(sys.argv)):
        names[i-1] = sys.argv[i]
